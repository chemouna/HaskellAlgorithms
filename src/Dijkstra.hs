...
-- we convert the list of edges to a map that lists all the neighbors
-- of each vertex.
...
-- Summary of the algorithm: start by giving all non-source vertices an infinite distance,
-- then go through all the vertices in order of their distance from the source, relaxing
-- all their neighbors.
